...
router.post('/register', async (req, res, next) => {
  try {
    ...
    res.status(201).json({ success: true, user: { _id: user._id, name: user.name, email: user.email, avatar: user.avatar } });
...
router.post('/login', async (req, res, next) => {
  try {
    ...
    res.cookie('accessToken', accessToken, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'strict', maxAge: 15 * 60 * 1000 });
    res.cookie('refreshToken', refreshToken, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'strict', maxAge: 7 * 24 * 60 * 60 * 1000 });
    res.json({ success: true, user: { _id: user._id, name: user.name, email: user.email, avatar: user.avatar, upiId: user.upiId } });
  } catch (error) { next(error); }
});
...
router.put('/profile', auth, async (req, res, next) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(req.user._id, { name, upiId, avatar }, { new: true, runValidators: true }).select('-password');
    res.json({ success: true, user });
  } catch (error) { next(error); }
});
